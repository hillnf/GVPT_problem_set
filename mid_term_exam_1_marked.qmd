...
::: {.callout-tip title="HG comments"}
The SD will increase. Remember, the SD measures the spread of the data. 
:::
...
::: {.callout-tip title="HG comments"}
The 40 year old cannot be the oldest person in the room. The median is the middle value of the vector of values (when arranged in order). Therefore, if there is a 40 year old and the median age is 50 years old, there must be people older than 50 in the room. 
...
![](images/Screen%20Shot%202023-10-06%20at%2012.15.11%20PM.png)
...
::: {.callout-tip title="HG comments"}
This is quite hard to read! Please make sure to format your cross tables in a more reader-friendly way. 
:::
...
::: {.callout-tip title="HG comments"}
You have cut the data in a way that makes testing this hypothesis difficult. Here's what I suggest: 

```{r}
modelsummary::datasummary_crosstab(social_trust ~ voted08, 
                                   data = tidyr::drop_na(poliscidata::gss, social_trust, voted08))
```

This suggests that the proportion of people who did vote increases with their level of social trust. Only 65.2% of people with the lowest level of social trust voted in this election. 88.9% of people with the highest level of social trust voted in this election. This suggests that as a individual's level of social trust increases, so too does the probability that they will vote.
...
::: {.callout-tip title="HG comments"}
Correct, but we can use the coefficient to interpret the effect of the independent variable more specifically. If the percentage of a state’s population that lives in an urban area increases by one percentage point, the percentage of the state’s population that says abortion should always be allowed increases by 0.373 percentage points, on average.
:::
...
::: {.callout-tip title="HG comments"}
The RSS gives us a good sense of how far our predicted values are from our observed values across our observations. 

To calculate the RSS of these states combined, we can do the following: 

```{r}
broom::augment(model, newdata = filter(states, stateid %in% c("CA    ", "TX    "))) |> 
...
::: {.callout-tip title="HG comments"}
OLS regression only measures the correlation between variables. It does not test for causation. 
:::
...
::: {.callout-tip title="HG comments"}
We should always use proportions when discussing the relationship between variables in groups. This prevents us from inflating the effect of large groups. There are more Dems than Reps and Independents. Therefore, if you look at the raw counts, you will overstate the level of confidence Dems have in each of the categories when comparing them to Reps. 

Here's what I recommend: 

```{r}
modelsummary::datasummary_crosstab(partyid ~ confed, data = gss)
```
